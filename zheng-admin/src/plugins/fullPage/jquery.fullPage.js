...
(function (global, factory) {
    ...
    $.fn.fullpage = function (options) {
        ...
        /**
         * Adds or remove the possiblity of scrolling through sections by using the mouse wheel/trackpad or touch
         * gestures. Optionally a second parameter can be used to specify the direction for which the action will be
         * applied.
         *
        ...
        function init() {
            ...
            //the active section isn't the first one? Is not the first slide of the first section? Then we load that
            // section/slide by default.
            ...
            if (!options.animateAnchor) {
                ...
                if (destiny.length) {
                    ...
                    if (section.length) {
                        ...
                        $.isFunction(options.afterLoad) && options.afterLoad.call(section, destiny,
        ...
        function styleSlides(section, slides, numSlides) {
            ...
            //the active section isn't the first one? Is not the first slide of the first section? Then we load that
            // section/slide by default.
            if (startingSlide.length && ($(SECTION_ACTIVE_SEL).index(SECTION_SEL) !== 0 || ($(SECTION_ACTIVE_SEL)
                                                                                                .index(SECTION_SEL)
                                                                                            === 0
                                                                                            && startingSlide.index()
                                                                                            !== 0))) {
        ...
        function styleMenu(section, index) {
            ...
            //moving the menu outside the main container if it is inside (avoid problems with fixed positions when
            // using CSS3 tranforms)
        ...
        function addVerticalNavigation() {
            ...
            nav.addClass(function () {
                return options.showActiveTooltip ? SHOW_ACTIVE_TOOLTIP + ' ' + options.navigationPosition
            ...
            for (var i = 0; i < $(SECTION_SEL).length; i++) {
                ...
                if (typeof tooltip !== 'undefined' && tooltip !== '') {
                    li +=
                        '<div class="' + SECTION_NAV_TOOLTIP + ' ' + options.navigationPosition + '">' + tooltip
                        + '</div>';
                }
            ...
            if (!options.autoScrolling || options.scrollBar) {
                ...
                if (!currentSection.hasClass(ACTIVE) && !currentSection.hasClass(AUTO_HEIGHT)) {
                    ...
                    if (canScroll) {
                        ...
                        $.isFunction(options.onLeave) && options.onLeave.call(leavingSection, leavingSectionIndex,
                                                                              sectionIndex, yMovement);
        ...
        */
        function touchMoveHandler(event) {
            ...
            // additional: if one of the normalScrollElements isn't within options.normalScrollElementTouchThreshold
            // hops up the DOM chain
        ...
        function scrollPage(element, callback, isMovementUp) {
            ...
            requestAnimFrame(function () {
                ...
                var dtop = element.hasClass(AUTO_HEIGHT) ? (dest.top == 0 ? 0 : dest.top - windowsHeight
                                                                                + element.height()) : dest.top;
                ...
                if ($.isFunction(options.onLeave) && !v.localIsResizing) {
                    if (options.onLeave.call(v.activeSection, v.leavingSection, (v.sectionIndex + 1), v.yMovement)
                        === false) {
                        return;
                    } else {
        ...
        function hashChangeHandler() {
            if (!isScrolling && !options.lockAnchors) {
                ...
                if (section.length) {
                    //when moving to a slide in the first section for the first time (first time to add an anchor to
                    // the URL)
        ...
        function landscapeScroll(slides, destiny) {
            ...
            if (options.css3) {
                ...
                addAnimation(slides.find(SLIDES_CONTAINER_SEL), options.scrollingSpeed > 0)
        ...
        function resizeHandler() {
            ...
            if (isTouchDevice) {
                ...
                if (!activeElement.is('textarea') && !activeElement.is('input') && !activeElement.is('select')) {
                    ...
                    //making sure the change in the viewport size is enough to force a rebuild. (20 % of the window to
                    // avoid problems when hidding scroll bars)
                    if (Math.abs(currentHeight - previousHeight) > (20 * Math.max(previousHeight, currentHeight)
                                                                    / 100)) {
                        FP.reBuild(true);
         ...
         * A class `fp-responsive` is added to the plugin's container in case the user wants to use it for his own
         * responsive CSS.
         */
        function responsive() {
        ...
        /**
         * Adds a css3 transform property to the container class with or without animation depending on the animated
         * param.
         */
